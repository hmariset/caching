...
Set `squid.namespace` and `nginx.namespace` to distinct, non-empty namespace names. Configure annotations separately with `squid.namespaceAnnotations` and `nginx.namespaceAnnotations`; `{}` or `null` applies no annotations. Test and mirrord resources use the Squid namespace.
